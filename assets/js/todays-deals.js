...
// Renders "Today's Top Deals" from language-specific data/products-<lang>.json (falls back to data/products.json) by selecting products where isDiscounted === true.
...
(function () {
 ...
 // Resolve correctly when hosted under a subpath (e.g., GitHub Pages).
...
function formatFreeShipText(o) {
 ...
 function buildTags(p, labels) {
  var out = [];
  // Match Products page tag labels
  if (labels.isLB) out.push('<span class="tag">Leaping Bunny</span>');
  if (labels.isPeta) out.push('<span class="tag">PETA</span>');
...
var labels = resolveLabels(p, brand);
   ...
   '<article class="dealCard">' +
    ...
    '<div class="dealTop">' +
     '<div class="dealBrandRow">' +
      '<div>' +
       '<div class="dealBrand">' + esc(brandName) + '</div>' +
 ...
 function main() {
  ...
  var lang = (window.kbwgGetLang ? window.kbwgGetLang() : ((new URLSearchParams(window.location.search||'')).get('lang') || 'he'));
  var productsRel = (window.kbwgPickLangJson ? window.kbwgPickLangJson('data/products.json', lang) : 'data/products.json');
  ...
  Promise.all([productsReq, brandsReq])
   .then(function (res) {
    ...
    if (!deals.length) {
     ...
     // Notify dynamic content rendered
     try { window.dispatchEvent(new Event('kbwg:content-rendered')); } catch (e) {}
     return;
    }
